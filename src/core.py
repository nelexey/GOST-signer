...
class SigningError(CryptoError): pass
...
def create_signature(curve, prv, dgst, filename='', filesize=0, mode=2018):  # Добавлен параметр mode
    signature = gost341012.sign(curve, prv, dgst, mode)
...
def verify_signature(dgst, s, own_pubkey=None, mode=2018):
    try:
        params = s.getComponentByName('params').getComponentByName('keydatasquence')
        ...
        return gost341012.verify(curve, pub, dgst, signature, mode)
